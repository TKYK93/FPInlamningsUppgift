...
class Tomteland {
    ...
    private val nissePair:Map<String,Array<String>>  = mapOf(
        "Tomten" to arrayOf("Glader", "Butter"),
        "Glader" to arrayOf("Tröger", "Trötter", "Blyger"),
        "Butter" to arrayOf("Rådjuret", "Nyckelpigan", "Haren", "Räven"),
        "Trötter" to arrayOf("Skumtomten"),
        "Skumtomten" to arrayOf("Dammråttan"),
        "Räven" to arrayOf("Gråsuggan","Myran"),
        "Myran" to arrayOf("Bladlusen"),
        )
    ...
    fun getUnderlings(currentName: String, res: MutableList<String>): List<String> {
        ...
        var matchedObject = nissePair[currentName]
        if (matchedObject == null) {
            return res
        }
        for(i in 0 until matchedObject.size){
            res.add(matchedObject[i])
            getUnderlings(matchedObject[i], res)
        }
        return res
    }
